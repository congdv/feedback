...
export default function CommentReaction({commentId, likeCount=0, dislikeCount=0, reactToComment, currentUserReact}: CommentReactionProps ) {
  ...
  return (
    <>
    <Button variant={"ghost"} onClick={handleLike} disabled={reaction === 'LIKE'}>
      <ThumbsUp color={`${reaction === 'LIKE' ? 'red' : "black"}`} className="h-4 w-4"/>
    </Button>
    <span>{like - dislike }</span>
    <Button variant={"ghost"} onClick={handleDislike} disabled={reaction=== 'DISLIKE'}>
      <ThumbsDown color={`${reaction === 'DISLIKE' ? 'red' : "black"}`} className="mr-2 h-4 w-4"/>
    </Button>
    </>
  )
}